...
本题中，设置一个阈值 $T \ge \sqrt n$，令 $g(n) = f(n) \ast f(n)$，并令 $F(n),G(n)$ 是它们的前缀和函数，先调和级数复杂度求出 $f(n)$ 和 $F(n)$ 的前 $T$ 项的值，利用如下公式求解余下点值的 $F(n)$：
$$
\begin{aligned}
F(n) =& \sum_{i=1}^n f(i) \newline
...
\end{aligned}
$$
预处理 $2^m$ 个 $\operatorname{lcm}$ 后，第一个和式可以在 $O(2^m)$ 时间内求出，后面所用到的 $f(d)$ 的 $d \le \sqrt n$，已经求出，所用到的 $F\left(\lfloor\dfrac{n}{d}\rfloor \right)$ 和 $F(\lfloor \sqrt n \rfloor)$ 也都已经求出，从而复杂度为 $O(\dfrac{n}{T}2^m+T \log T + \dfrac{n}{\sqrt T})$。
