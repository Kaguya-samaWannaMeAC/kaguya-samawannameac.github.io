...
考虑容斥，假设所有的数左端点均从 $1$ 开始，后 $a$ 个区间的违法区间为 $[1, i - n + a]$ 。设 $f_s$ 后 $a$ 个区间有 $s$ 个区间违法的违法方案数，因此前 $n - a$ 个区间的方案数为 $(a - s) ^ {n - a}$，后 $a$ 个区间的方案数为 $(a - s)!$，因此这种情况对答案的贡献为 $(-1)^{s}\times f_s \times (a - s) ^{n - a} \times (a - s) !$
