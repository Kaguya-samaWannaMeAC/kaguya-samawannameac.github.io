...
随便选一个节点当起点，设该点的字符串为 $s$，接着遍历欧拉回路，依次将边上的字符添加到 $s$ 后面。将最后得到的字符串看作环，每个字符串 $s \in {\{0,1,\ldots,m-1\}}^n$ 恰好出现了一次。这是因为图中**每个点**和**一条指出去的边**都唯一确定了一个 $s \in {\{0,1,\ldots,m-1\}}^n$，一共有 $m^n$ 个这样的二元组，对应 $m^n$ 个字符串。遍历整个欧拉回路的过程相当于不断滑动长度为 $n-1$ 的窗口。
